...
function KatarinaPonche:Combo(target)
	...
	elseif Menu.Combo.R:Value() and self:IsReady(_R) and (not self:IsReady(_Q) or not Menu.Combo.Q:Value()) and (not self:IsReady(_W) or not Menu.Combo.W:Value())
		and (not self:IsReady(_E) or not Menu.Combo.E:Value()) and getdmg("R", target, myHero) >= target.health then
...
function KatarinaPonche:Harass(target)
